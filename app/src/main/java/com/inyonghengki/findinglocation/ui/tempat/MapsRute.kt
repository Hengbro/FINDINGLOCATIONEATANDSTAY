...
class MapsRute : AppCompatActivity() {
    ...
    private lateinit var endLocation: Point
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_mapsrute)
    ...
    private fun openPlaceCard(suggestion: PlaceAutocompleteSuggestion) {
        ...
        lifecycleScope.launchWhenStarted {
            placeAutocomplete.select(suggestion).onValue { result ->
                val destinationPosition = result.coordinate
                LocationEngineProvider.getBestLocationEngine(applicationContext).userDistanceTo(this@MapsRute, destinationPosition) { distance ->
                    if (distance != null) {
                        // Lakukan sesuatu dengan nilai jarak yang diterima
                        Log.d(LOG_TAG, "User distance to destination: $distance meters")
                    } else {
                        // Tidak dapat mendapatkan lokasi pengguna
                        Log.d(LOG_TAG, "Failed to get user location")
                    }
                }
                ...
                //getDirectionsRoute(suggestion.coordinate, result.coordinate)
    ...
    private fun getDirectionsRoute(origin: Point, Destination:Point) {
        ...
        client.enqueueCall(object : Callback<DirectionsResponse> {
            override fun onResponse(call: Call<DirectionsResponse>, response: Response<DirectionsResponse>) {
                ...
                if (response.body() == null) {
                    Log.d(LOG_TAG,"No routes found, make sure you set the right user and access token.")
                    return
                } else if (response.body()!!.routes().size < 1) {
                    Log.d(LOG_TAG,"No routes found")
                    return
                }
            ...
            override fun onFailure(call: Call<DirectionsResponse>, throwable: Throwable) {
                ...
                Log.d(LOG_TAG,"Error: " + throwable.message)
    ...
    private fun drawRoute(route: DirectionsRoute) {
        val directionsRouteFeature = Feature.fromGeometry(currentRoute.geometry()
            ?.let { LineString.fromPolyline(it, PRECISION_6) })
    }
